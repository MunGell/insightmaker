...
function loadUnits(addedSources, addedTargets, addedScalings){
	...
	for (var uk = 0; uk < conTargets.length; uk++) {
  		conUnitTargets.push(new UnitStore(null, 1));
  		conUnitTargets[conUnitTargets.length - 1].fromString(conTargets[uk].toLowerCase());
		//console.log(conUnitTargets[conUnitTargets.length - 1]);
...
var findSourceIndex = function(name){
	if(name in findSourceIndex){
		return findSourceIndex[name];//memonization
	}
	for(var i = conSourceNames.length-1; i >= 0 ; i--){
		if(conSourceNames[i] != "" && conSourceRegEx[i].test(name)){
			findSourceIndex[name]=i;
			return i;
		}
	}
	findSourceIndex[name]=-1;
	return -1;
}
...
Material.prototype.forceUnits = function(newUnits){
	...
	if (scale == 0) {
		...
		newM.value = fn["*"](this.value, scale);
		newM.units = newUnits.clone();
	}
	return newM;
}
