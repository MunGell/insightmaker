...
TaskQueue.prototype.moveTime = function(timeChange){
	if(this.debug){
		console.log("Shifting time by: "+timeChange);
	}
	this.moveTo(plus(this.time, timeChange));
}
...
TaskQueue.prototype.moveTo = function(newTime){
	if(eq(this.time, newTime)){
		...
		if(this.debug){
			console.log("Shifting time to: " + newTime);
		}
		...
		if(this.debug){
			console.log("Time shift to  "+newTime+" completed.");
		}
	}
}
...
Task.prototype.rollback = function(){
	if(this.reverse && (! this.deadReverse) && ((! this.blocker) || ! this.queue.states[this.blocker]) ){
		if(this.queue.debug){
			console.log("Rolling back: "+this.name+" (Time: "+this.time+")");
		}
